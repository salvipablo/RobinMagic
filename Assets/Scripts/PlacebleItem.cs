...
public class PlacebleItem : MonoBehaviour
{
  ...
  #region Methods
  private void Start() => outline = GetComponent<Outline>();
  ...
  void Update()
  {
    ...
    if (Physics.Raycast(transform.position, Vector3.down, out groundHit, boxHeight * 0.5f, LayerMask.GetMask("Ground"))) isGrounded = true;
  ...
  private void OnTriggerEnter( Collider other )
  {
    ...
    if (other.CompareTag("Tree") || other.CompareTag("pickable")) isOverlappingItems = true;
  }

  private void OnTriggerExit( Collider other )
  {
    if (other.CompareTag("Ground") && PlacementSystem.Instance.inPlacementMode)isGrounded = false;
    if (other.CompareTag("Tree") || other.CompareTag("pickable") && PlacementSystem.Instance.inPlacementMode) isOverlappingItems = false;
  }
  #endregion
